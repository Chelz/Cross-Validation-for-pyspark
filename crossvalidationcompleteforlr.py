#Cross Validation Code for Logistic Regression
def CrossValidationforLR(k):
  countexcess={}
  countunder={}
...
#Labeled Data for Test Data
      ...
      model = LogisticRegressionWithSGD.train(labeled1_train,regParam=0.1)
      predictionAndLabeltrainsgd = labeled1_train.map(lambda p: (model.predict(p.features), p.label))
      accuracytrainsgd = 1.0 * predictionAndLabeltrainsgd.filter(lambda x: x[0] == x[1]).count() / train.count()
      print('model Train accuracy for Logistic Regression with SGD {}'.format(accuracytrainsgd))
      accuracy_sgd_train.append(accuracytrainsgd)
